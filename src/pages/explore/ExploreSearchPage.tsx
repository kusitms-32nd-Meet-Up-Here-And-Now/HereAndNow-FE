...
import { useSearchCourseList } from '@apis/explore/explore';
import type { CourseSearchParams } from '@apis/explore/explore';
...
const ExploreSearchPage = () => {
  ...
  // 검색 파라미터 변환 함수
  const buildSearchParams = (): CourseSearchParams => {
  ...
  // useSearchCourseList hook (enabled: false로 초기 설정)
  const { refetch: searchCourse } = useSearchCourseList(searchParams, false);
  ...
  const handleSearchClick = async () => {
    try {
      ...
      if (result.data?.isSuccess) {
        // 검색 성공 시 ExploreCoursePage로 navigate (location.state로 검색 파라미터 전달)
        navigate('/explore/course', { state: { searchParams, searchResult: result.data } });
      }
    } catch (error) {
